...
local GUI = cf("frame", nil, UIParent);
-----------------------------------------------
...
local function CreateCheckBox(frame, t, settings, callback)
	local CheckButton = cf("CheckButton", nil, frame, "OptionsBaseCheckButtonTemplate");
...
local function CreateBlankSquare(frame)
	local Frame = cf("frame", nil, frame);
	Frame:SetSize(26, 26);
	Frame:Show();
...
local function CreateEditBox(frame, t, settings, callback)
	local EditBox = cf("EditBox", nil, frame);
	ni.backend.ProtectFrame(EditBox)
	if t.width then
...
local function CreateInput(frame, t, settingsfile, callback)
	...
	local id = #f.items + 1;
	local TempFrame = cf("frame", nil, f);
	...
	if t.wordwrap then
		...
		scroll_frame = cf("ScrollFrame", scroll_frame_name, TempFrame, "UIPanelScrollFrameTemplate");
...
local function CreateMenu(frame, t, settings, callback)
	local DropDownMenu = cf("Button", nil, frame);
...
local function CreatePage(frame, page, t)
	local Frame = cf("frame", nil, frames[frame]);
	Frame.page = page;
	Frame.ispage = true;
	Frame:SetHeight(16);
	Frame:SetPoint("LEFT", 0, 0);
	Frame:SetPoint("RIGHT", 0, 0);
	local LeftButton = cf("Button", nil, Frame);
	...
	local RightButton = cf("Button", nil, Frame);
...
local function CreateIconFormator(frame)
	...
	local id = #f.items + 1;
	local iconformator = cf("frame", nil, f);
...
local function CreateSeparator(frame)
	...
	local id = #f.items + 1;
	local separator = cf("frame", nil, f);
...
local LeftButton = cf("Button", nil, GUI);
LeftButton:RegisterForClicks("LeftButtonUp");
LeftButton:SetSize(24, 24);
...
local RightButton = cf("Button", nil, GUI);
RightButton:RegisterForClicks("LeftButtonUp");
RightButton:SetSize(24, 24);
